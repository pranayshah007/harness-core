...
public class GcpSMConnectorDTOTest extends CategoryTest {
  ...
  public void testInputFieldsHaveCorrectValue() throws IllegalAccessException {
    ...
    // Create connector dto.
    GcpSMConnectorDTO connectorDTO = GcpSMConnectorDTO.builder().credentials(secretRefData).isDefault(false).build();
    // Get all the fields in it
    Field[] fields = GcpSMConnectorDTO.class.getDeclaredFields();
    assertThat(fields.length).isEqualTo(3);
    assertThat(connectorDTO).isNotNull();
    assertThat(SecretRefHelper.getSecretConfigString(connectorDTO.getCredentials())).isEqualTo("some-secret");
