...
public class JschAgent extends SshAgent<JschClient, JschExecSession, JschSftpSession> {
  ...
  protected ExecResponse exec(ExecCommandData commandData) {
    ...
    try (SshClient client = getClient()) {
      ...
      if (CHANNEL_IS_NOT_OPENED.equals(jsch.getMessage())) {
        ...
        return ExecResponse.builder().response(output).exitCode(1).status(commandExecutionStatus).build();
      }
    } catch (Exception ex) {
      handleException(ex);
  ...
  @Override
  protected JschClient getClient() {
    Session session = getJschSession();
    return JschClient.builder().session(session).build();
  }

  @Override
  protected JschExecSession getExecSession(SshClient sshClient) {
    try {
  ...
  @Override
  protected JschSftpSession getSftpSession(SshClient sshClient) {
    try {
  ...
  private static void generateTGTUsingSshConfig(SshSessionConfig config, LogCallback logCallback) throws JSchException {
