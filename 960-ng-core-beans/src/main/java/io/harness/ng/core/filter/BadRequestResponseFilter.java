...
public class BadRequestResponseFilter implements ContainerResponseFilter {
  ...
  public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext) {
    if(BAD_REQUEST.getStatusCode() == responseContext.getStatus()
    && responseContext.getEntity() instanceof FailureDTO
    && requestContext.getUriInfo().getPath().startsWith("v1")) {
      ...
      List<BadRequestDTO.BadRequestDetailDTO> details = failureDTO.getErrors()
              .stream()
