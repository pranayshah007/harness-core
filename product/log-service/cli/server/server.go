...
func GPTRCAThread(ctx context.Context, store store.Store, keyErrorMsg types.KeyErrorMsg, OpenAPIToken string) {
	...
	reqContent := "I have some errors. Can you please tell me the root cause, possible solution and error category in the following format? \n\nError:\nCause:\nPossible Solution:\nCategory:\n\nError Category must be classified from one of the following:\nError messages: These are the most obvious patterns to look for. They may include syntax errors, type errors, runtime errors, etc. Examples include \"SyntaxError\", \"TypeError\", \"NameError\", \"ValueError\", etc.\n\nAssertion failures: Assertion failures occur when a test fails. These failures often include a message that explains what went wrong. Examples include \"assertion failed\", \"assertion error\", \"test failed\", etc.\n\nFile not found errors: These occur when a file that the build process depends on cannot be found. Examples include \"No such file or directory\", \"File not found\", \"Can't open file\", etc.\n\nNetwork errors: These occur when the build process depends on external resources (e.g., APIs) and there are issues with connectivity or authentication. Examples include \"Connection refused\", \"Connection timed out\", \"401 Unauthorized\", etc.\n\nMemory errors: These occur when the build process runs out of memory. Examples include \"Out of memory\", \"MemoryError\", \"malloc: out of memory\", etc.\n\nPermissions errors: These occur when the build process does not have the required permissions to access a resource. Examples include \"Permission denied\", \"Access denied\", \"Operation not permitted\", etc.\n\nDependency issues: These occur when the build process depends on a library or package that is not installed or is incompatible with the current environment. Examples include \"ModuleNotFoundError\", \"ImportError\", \"Dependency not found\", etc.\n\nConfiguration errors: These occur when the build process is not configured correctly. Examples include \"Invalid configuration\", \"Configuration error\", \"Missing configuration\", etc.\n\nHere is the error message:" + keyErrorMsg.ErrorMsg
