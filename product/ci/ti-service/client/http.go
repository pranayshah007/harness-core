...
const (
	...
	getTestsTimesEndpoint = "/tests/timedata?accountId=%s&orgId=%s&projectId=%s&pipelineId=%s&buildId=%s&repo=%s"
...
// GetTestTimes gets test times
func (c *HTTPClient) GetTestTimes(ctx context.Context, org, project, pipeline, build, repo, reqBody string) (types.GetTestTimesResp, error) {
	ctx = context.WithValue(ctx, "reqId", "")
	path := fmt.Sprintf(getTestsTimesEndpoint, c.AccountID, org, project, pipeline, build, repo)
