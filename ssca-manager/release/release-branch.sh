...
git log --remotes=origin/release/${PURPOSE}/* --pretty=oneline --abbrev-commit | grep -iE "\[SSCA-[0-9]+]:" -o | sort | uniq > release.txt
git log --remotes=origin/$CURRENT_BRANCH --pretty=oneline --abbrev-commit | grep -iE "\[SSCA-[0-9]+]:" -o | sort | uniq > develop.txt
