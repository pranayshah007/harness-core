...
public class SecurityStagePMSPlanCreator extends GenericStagePlanCreator<SecurityStageNode> {
  ...
  public LinkedHashMap<String, PlanCreationResponse> createPlanForChildrenNodes(
      PlanCreationContext ctx, SecurityStageNode stageElementConfig) {
    log.info("Received plan creation request for security stage {}", stageElementConfig.getIdentifier());
  ...
  public StepType getStepType(SecurityStageNode stageElementConfig) {
    return SecurityStageStepPMS.STEP_TYPE;
  }

  @Override
  public SpecParameters getSpecParameters(
      String childNodeId, PlanCreationContext ctx, SecurityStageNode stageElementConfig) {
    ExecutionSource executionSource = buildExecutionSource(ctx, stageElementConfig);
    BuildStatusUpdateParameter buildStatusUpdateParameter =
        obtainBuildStatusUpdateParameter(ctx, stageElementConfig, executionSource);
  ...
  private ExecutionElementConfig modifyYAMLWithImplicitSteps(PlanCreationContext ctx, ExecutionSource executionSource,
      YamlField executionYAMLField, SecurityStageNode stageElementConfig) {
  ...
  private ExecutionSource buildExecutionSource(PlanCreationContext ctx, SecurityStageNode stageElementConfig) {
  ...
  private BuildStatusUpdateParameter obtainBuildStatusUpdateParameter(
      PlanCreationContext ctx, SecurityStageNode stageElementConfig, ExecutionSource executionSource) {
    CodeBase codeBase = getCICodebase(ctx);
