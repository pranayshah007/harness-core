    ...
    if (parentPath.isPresent() && VALUES.equals(parentPath.get().getLast())) {
      if (environmentYamlV2.getEnvironmentRef().isExpression()) {
        ...
        */
        ...
        metadata.put(PreFlightCheckMetadata.YAML_TYPE_REF_NAME, "environmentRef");
        ...
        fullQualifiedDomainName = VisitorParentPathUtils.getFullQualifiedDomainName(contextMap) + PATH_CONNECTOR
            + environmentYamlV2.getEnvironmentRef().getValue() + PATH_CONNECTOR + "environmentRef";
      }
    } else {
      fullQualifiedDomainName =
          VisitorParentPathUtils.getFullQualifiedDomainName(contextMap) + PATH_CONNECTOR + "environmentRef";
    }
