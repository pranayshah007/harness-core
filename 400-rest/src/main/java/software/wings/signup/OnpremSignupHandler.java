...
public class OnpremSignupHandler implements SignupHandler {
  ...
  public boolean handle(UserInvite userInvite) {
    ...
    if (defaultAccount.isPresent()) {
      userService.createNewUserAndSignIn(mapUserInviteToUser(userInvite, defaultAccount.get()), defaultAccount.get().getUuid());
    } else {
  ...
  private void validateCountOfUsers() {
    if (getUserCount() > 0) {
      throw new SignupException("One or more users already exist. Onprem environment does not allow multiple user signup");
    }
  }
