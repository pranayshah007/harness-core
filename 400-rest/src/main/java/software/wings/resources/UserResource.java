...
public class UserResource {
  ...
  @PUT
  @Path("userAccountLevelPreference/{userId}")
  @Timed
  @ExceptionMetered
  public RestResponse<User> updateUserAccountLevelPreference(@QueryParam("accountId") @NotEmpty String accountId,
      @PathParam("userId") String userId, @NotNull UserPreferenceDataDTO userPreferenceDataDTO) {
    User authUser = UserThreadLocal.get();
    if (!authUser.getUuid().equals(userId)) {
      throw new WingsException(ErrorCode.ACCESS_DENIED, USER);
    }
    authUser.setUuid(userId);
    if (isEmpty(authUser.getAppId())) {
      authUser.setAppId(GLOBAL_APP_ID);
    }
    return new RestResponse<>(userService.updateUserAccountLevelPreference(authUser, accountId, userPreferenceDataDTO));
    ...
    public RestResponse delete(
        @QueryParam("accountId") @NotEmpty String accountId, @PathParam("userId") String userId) {
    ...
    public RestResponse<User> completeMarketPlaceInvite(@QueryParam("account") @NotEmpty String accountName,
        ...
        @QueryParam("marketPlaceType") MarketPlaceType marketPlaceType,
        @PathParam("inviteId") @NotEmpty String inviteId, @NotNull UserInvite userInvite) {
      ...
      try (AutoLogContext ignore1 = new MarketplaceTypeLogContext(marketPlaceType, OVERRIDE_ERROR)) {
        ...
        try {
          ...
          log.info("Account Name and Company Name must be UTF-8 compliant. accountName: {} companyName: {}",
