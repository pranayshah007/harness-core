...
public class DeploymentReconTaskV2 implements Runnable {
  ...
  public DeploymentReconTaskV2(AccountService accountService, DeploymentEventProcessor deploymentEventProcessor,
      FeatureFlagService featureFlagService, PersistentLocker persistentLocker,
      Set<ExecutionEntity<?>> executionEntities, Account account, ExecutorService executorService) {
    ...
    try {
      ...
      for (ExecutionEntity executionEntity : executionEntities) {
        if (!DeploymentExecutionEntity.SOURCE_ENTITY_CLASS.equals(executionEntity.getSourceEntityClass())
            && !featureFlagService.isEnabled(FeatureName.STAGE_AND_STEP_LEVEL_DEPLOYMENT_DETAILS, account.getUuid())) {
          continue;
        }
