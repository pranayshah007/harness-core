...
public class DelegateServiceGrpcImpl extends DelegateServiceImplBase {
  ...
  private TaskData createTaskData(TaskDetails taskDetails) {
    ...
    if (taskDetails.getParametersCase().equals(TaskDetails.ParametersCase.KRYO_PARAMETERS)) {
      ...
      software.wings.beans.TaskType type = software.wings.beans.TaskType.valueOf(String.valueOf(taskType.getType()));
      try {
        TaskParameters taskParameters = objectMapper.readValue(
                taskDetails.getJsonParameters().toStringUtf8(), type.getRequest());
