...
public class UserServiceImplTest extends WingsBaseTest {
  ...
  public void testTrialSignup() {
    ...
    when(signupService.getUserInviteByEmail(Matchers.eq(email)))
            .thenReturn(null);
    userServiceImpl.trialSignup(userInvite);
    // Verifying that the mail is sent and event is published when a new user
    // sign ups for trial
    Mockito.verify(accountService, times(1))
            .validateAccount(any(Account.class));
    Mockito.verify(signupService, times(1))
            .sendTrialSignupVerificationEmail(any(), any());
    Mockito.verify(eventPublishHelper, times(1))
  ...
  public void testGetUserCountForPendingAccounts() {
    ...
    assertThat(userServiceImpl.getTotalUserCount("ACCOUNT_ID", true))
            .isEqualTo(1);
  }
  ...
  public void test_checkInviteStatus_userAddedRequiredPassword() {
    ...
    UserInvite userInviteRetrieved =
            userServiceImpl.getUserInviteByEmailAndAccount(USER_EMAIL, ACCOUNT_ID);
    InviteOperationResponse inviteOperationResponse =
            userServiceImpl.checkInviteStatus(userInviteRetrieved, Generation.CG);
    assertThat(inviteOperationResponse)
            .isEqualTo(ACCOUNT_INVITE_ACCEPTED_NEED_PASSWORD);
    verify(eventPublishHelper, times(1))
            .publishUserInviteVerifiedFromAccountEvent(withNonSso.getUuid(),
                    USER_EMAIL);
  }
  ...
  public void test_checkInviteStatus_userInviteAccepted() {
    ...
    verify(eventPublishHelper, times(0))
            .publishUserInviteVerifiedFromAccountEvent(withNonSso.getUuid(),
                    USER_EMAIL);
  }
  ...
  public void test_checkInviteStatus_userInvalid() {
    ...
    when(accountService.get(ACCOUNT_ID)).thenReturn(withNonSso);
    UserInvite userInviteRetrieved =
            userServiceImpl.getUserInviteByEmailAndAccount("spam@harness.io",
                    ACCOUNT_ID);
    InviteOperationResponse inviteOperationResponse =
            userServiceImpl.checkInviteStatus(userInviteRetrieved, Generation.CG);
  ...
  public void test_checkInviteStatus_userInviteInvalid() {
    ...
    when(accountService.get(ACCOUNT_ID)).thenReturn(withNonSso);
    InviteOperationResponse inviteOperationResponse =
            userServiceImpl.checkInviteStatus(userInvite, Generation.CG);
    assertThat(inviteOperationResponse).isEqualTo(INVITE_INVALID);
    verify(eventPublishHelper, times(0))
            .publishUserInviteVerifiedFromAccountEvent(withNonSso.getUuid(),
                    USER_EMAIL);
  ...
  public void test_checkInviteStatus_SsoUserAdded() {
    ...
    when(harnessCacheManager.getCache(
            PRIMARY_CACHE_PREFIX + USER_CACHE, String.class, User.class,
    ...
    verify(eventPublishHelper, times(1))
            .publishUserInviteVerifiedFromAccountEvent(withSso.getUuid(),
                    USER_EMAIL);
  }
  ...
  public void test_CompleteNgInvite_2FA() throws Exception {
    ...
    when(harnessCacheManager.getCache(
            ...
            .thenReturn(new NoOpCache<>());
    userServiceImpl.completeNGInvite(inviteDTO, false, true);
    verify(totpAuthHandler, times(1))
            .sendTwoFactorAuthenticationResetEmail(any());
    verify(eventPublishHelper, times(1))
            .publishUserRegistrationCompletionEvent(anyString(), any());
  }
  ...
  test_getInviteAcceptRedirectURL_AccountInviteAcceptedNeedPassword()
    ...
    String accountCreationFragment =
            String.format("accountIdentifier=%s&email=%s&token=%s&generation=CG",
                    ACCOUNT_ID, encodedEmail, jwtToken);
    ...
    assertThat(userServiceImpl.getInviteAcceptRedirectURL(
            inviteOperationResponse, userInvite, jwtToken))
  ...
  public void shouldSortUsers() {
    ...
    List<User> userList = userServiceImpl.listUsers(pageRequest, "ACCOUNT_ID",
            "", 0, 30, false, true);
    assertThat(userList.get(2).getName()).isEqualTo("pqr");

    map.put("sort[0][field]", Arrays.asList("email"));
    when(uriInfo.getQueryParameters(true)).thenReturn(map);
    userList = userServiceImpl.listUsers(pageRequest, "ACCOUNT_ID", "", 0, 30,
            false, true);
  ...
  public void shouldSearchUsers() {
    ...
    userList = userServiceImpl.listUsers(pageRequest, "ACCOUNT_ID", "PqR", 0,
            30, false, true);
  ...
  public void shouldSearchAndSortUsers() {
    ...
    userList = userServiceImpl.listUsers(pageRequest, "ACCOUNT_ID", "fgh", 0,
            30, false, true);
  ...
  public void test_gerUseraccountIds() {
    ...
    Account account2 =
            anAccount().withUuid("id1").withAccountName("account2").build();
  ...
  public void test_getUsersAccounts() {
    ...
    HarnessUserGroup harnessUserGroup = HarnessUserGroup.builder()
            .uuid("groupId")
            .memberIds(Sets.newHashSet("user1"))
            .build();
  ...
  public void sanitizeUserNameTest_without_malicious_content() {
    ...
    assertThat(userServiceImpl.sanitizeUserName(name3))
  ...
  public void sanitizeUserNameTest_with_malicious_content() {
    ...
    String name2 =
            "'''><img src=x onerror=alert(1)> '''><img srx onerror=alert(1)>";
    String name3 = "</h2>special offer <a href=www.attacker.site>" + USER_NAME +
            "</a><h2>";
    ...
    assertThat(userServiceImpl.sanitizeUserName(name2))
  ...
  public void test_delete_when_user_part_of_only_one_CG_account() {
    ...
    UserGroup userGroup1 =
            builder()
                    ...
                    .memberIds(new LinkedList<>(
                            Arrays.asList(user1.getUuid(), user2.getUuid())))
                    .members(new LinkedList<>(Arrays.asList(user1, user2)))
                    .isDefault(true)
                    .build();
    UserGroup userGroup1_updated =
            builder()
                    .accountId("ACCOUNT_ID")
                    .uuid(UUIDGenerator.generateUuid())
                    .name("abcug" + System.currentTimeMillis())
                    ...
                    .build();
    wingsPersistence.save(user1);
    when(harnessUserGroupService.isHarnessSupportUser(user1.getUuid()))
            .thenReturn(false);
    ...
    when(userGroupService.getCountOfUserGroups("ACCOUNT_ID"))
            .thenReturn(Long.valueOf(1));
    when(userGroupService.list(
            "ACCOUNT_ID",
            aPageRequest()
                    .withLimit("1")
                    .addFilter(UserGroupKeys.memberIds, HAS, user1.getUuid())
                    .build(),
            true, null, null))
            .thenReturn(aPageResponse()
                    .withResponse(Collections.singletonList(userGroup1))
                    .withTotal(1)
                    .withLimit("10")
                    .build());
    when(userGroupService.updateMembers(userGroup1, false, false))
            .thenReturn(userGroup1_updated);
    when(configurationController.isPrimary()).thenReturn(true);
    when(harnessCacheManager.getCache(
            PRIMARY_CACHE_PREFIX + USER_CACHE, String.class, User.class,
            AccessedExpiryPolicy.factoryOf(Duration.THIRTY_MINUTES)))
    ...
    when(eventProducer.send(
            Message.newBuilder()
                    .putAllMetadata(ImmutableMap.of(
                            ...
                            EventsFrameworkMetadataConstants.USER_ENTITY,
    ...
    assertThat(wingsPersistence.createQuery(UserInvite.class)
            ...
            .get())
            .isEqualTo(null);
    when(userServiceHelper.isUserActiveInNG(any(), anyString()))
            .thenReturn(false);
    ...
    assertThat(wingsPersistence.findAndDelete(
            wingsPersistence.createQuery(User.class)
                    .filter(BaseKeys.uuid, user1.getUuid()),
  ...
  public void test_delete_when_user_part_of_only_one_NG_account()
    ...
    when(userMembershipClient.isUserInScope(user1.getUuid(), "ACCOUNT_ID", null,
            null))
    ...
    when(userMembershipClient.removeUserInternal(
            user1.getUuid(), "ACCOUNT_ID", null, null,
            NGRemoveUserFilter.ACCOUNT_LAST_ADMIN_CHECK))
            .thenReturn(responseDTOCall);
    ...
    doNothing()
            ...
            .reportDeleteForAuditingUsingAccountId("ACCOUNT_ID", user1);
    doNothing()
            .when(userServiceHelper)
            .deleteUserFromNG("ACCOUNT_ID", user1.getUuid(),
                    NGRemoveUserFilter.ACCOUNT_LAST_ADMIN_CHECK);
    ...
    verify(userServiceHelper, times(1))
            .updatedActiveAccounts(any(), anyString());
    verify(userServiceHelper, times(1))
            .updatedPendingAccount(any(), anyString());
    verify(userServiceHelper, times(1))
            .deleteUserFromNG(anyString(), anyString(), any());
    verify(userGroupService, times(1))
            .list("ACCOUNT_ID",
                    aPageRequest()
                            .withLimit("1")
