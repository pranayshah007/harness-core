...
yq -i 'del(.spec.template.spec.containers[0].startupProbe.httpGet)' $DEPLOY_FILE
yq -i '.spec.template.spec.containers[0].startupProbe.exec.command |= ["ls"]' $DEPLOY_FILE
...
## DEPLOY
echo "Deploying (it really takes time) [$NAMESPACE/$POD_NAME]"
kubectl cp ~/.bazel-dirs/bin/360-cg-manager/module_deploy.jar $NAMESPACE/$POD_NAME:/opt/harness/rest-capsule.jar
