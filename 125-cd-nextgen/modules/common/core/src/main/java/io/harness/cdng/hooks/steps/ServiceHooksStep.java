...
public class ServiceHooksStep extends NGForkStep {
  ...
  public StepResponse handleChildrenResponse(
    ...
    ServiceHooksOutcome serviceHooksOutcome = new ServiceHooksOutcome();
    if (EmptyPredicate.isNotEmpty(outcomes)) {
      for (Outcome outcome : outcomes) {
        if (!(outcome instanceof ServiceHookOutcome)) {
          continue;
        }

        ServiceHookOutcome serviceHookOutcome = (ServiceHookOutcome) outcome;
        serviceHooksOutcome.put(serviceHookOutcome.getIdentifier(), serviceHookOutcome);
      }
    }
