...
import static io.harness.cdng.pipeline.helpers.ExecutionStrategyTemplates.SSH_WINRM_BASIC_SH_FTL_V3;
import static io.harness.cdng.pipeline.helpers.ExecutionStrategyTemplates.SSH_WINRM_CANARY_SH_FTL_V3;
import static io.harness.cdng.pipeline.helpers.ExecutionStrategyTemplates.SSH_WINRM_ROLLING_SH_FTL_V3;
...
@Slf4j
@OwnedBy(CDP)
public class CDNGPipelineExecutionStrategyHelperV3 {
  ...
  @Inject
  public CDNGPipelineExecutionStrategyHelperV3(CDNGPipelineExecutionStrategyHelper cdngPipelineExecutionStrategyHelper,
  ...
  protected String generateSshWinRmBasicYaml(String accountIdentifier, ServiceDefinitionType serviceDefinitionType,
    ...
    try (StringWriter stringWriter = new StringWriter()) {
      ...
      String basicSnippet = Resources.toString(Objects.requireNonNull(classLoader.getResource(
                                                   format("snippets/Pipelines/execution/v3/basic/%s/%s-basic-%s%s.yaml",
      ...
      Map<String, Object> templateParams = ImmutableMap.<String, Object>builder()
                                               ...
                                               .build();
      try {
        ExecutionStrategyTemplates.getTemplate(SSH_WINRM_BASIC_SH_FTL_V3).process(templateParams, stringWriter);
  ...
  protected String generateSshWinRmRollingYaml(String accountIdentifier, ServiceDefinitionType serviceDefinitionType,
    ...
    try (StringWriter stringWriter = new StringWriter()) {
      ...
      String rollingSnippet = Resources.toString(Objects.requireNonNull(classLoader.getResource(format(
                                                     "snippets/Pipelines/execution/v3/rolling/%s/%s-rolling-%s%s.yaml",
                                                     serviceDefinitionType.name().toLowerCase(Locale.ROOT),
                                                     serviceDefinitionType.name().toLowerCase(Locale.ROOT),
                                                     artifactType, includeVerify ? "-with-verify" : ""))),
          StandardCharsets.UTF_8);
      Map<String, Object> templateParams =
          ImmutableMap.<String, Object>builder()
              ...
              .build();
      try {
        ExecutionStrategyTemplates.getTemplate(SSH_WINRM_ROLLING_SH_FTL_V3).process(templateParams, stringWriter);
      } catch (TemplateException te) {
        throw new InvalidArgumentsException("Error processing yaml template: " + te.getMessage());
      }
      return stringWriter.toString().trim();
  ...
  protected String generateSshWinRmCanaryYaml(String accountIdentifier, ServiceDefinitionType serviceDefinitionType,
    ...
    try (StringWriter stringWriter = new StringWriter()) {
      ...
      String canarySnippet = Resources.toString(Objects.requireNonNull(classLoader.getResource(format(
                                                    "snippets/Pipelines/execution/v3/canary/%s/%s-canary-%s%s.yaml",
                                                    serviceDefinitionType.name().toLowerCase(Locale.ROOT),
      ...
      String canaryRollbackSnippet = Resources.toString(
          Objects.requireNonNull(
              classLoader.getResource(format("snippets/Pipelines/execution/v3/canary/%s/%s-canary-%s%s-rollback.yaml",
                  serviceDefinitionType.name().toLowerCase(Locale.ROOT),
                  serviceDefinitionType.name().toLowerCase(Locale.ROOT), artifactType,
                  includeVerify ? "-with-verify" : ""))),
          StandardCharsets.UTF_8);
      Map<String, Object> templateParams =
          ImmutableMap.<String, Object>builder()
              ...
              .build();
      try {
        ExecutionStrategyTemplates.getTemplate(SSH_WINRM_CANARY_SH_FTL_V3).process(templateParams, stringWriter);
      } catch (TemplateException te) {
        throw new InvalidArgumentsException("Error processing yaml template: " + te.getMessage());
      }
      return stringWriter.toString().trim();
