...
public class ServiceHooksStepTest extends CDNGTestBase {
  ...
  @Inject private ServiceHooksStep serviceHooksStep;
  ...
  public void testHandleChildrenResponse() {
    ...
    when(serviceStepsHelper.getChildrenOutcomes(responseData))
        .thenReturn(Collections.singletonList(ServiceHookOutcome.builder().identifier(identifier).build()));
    when(executionSweepingOutputService.listOutputsWithGivenNameAndSetupIds(
             any(), eq(FAILED_CHILDREN_OUTPUT), anyList()))
        .thenReturn(Collections.emptyList());

    StepResponse response =
        serviceHooksStep.handleChildrenResponse(ambiance, ForkStepParameters.builder().build(), responseData);
    ...
    ServiceHooksOutcome serviceHooksOutcome = (ServiceHooksOutcome) stepOutcomes[0].getOutcome();
    ServiceHookOutcome serviceHookOutcome = serviceHooksOutcome.get(identifier);
