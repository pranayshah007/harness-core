...
@Slf4j
public class EcsBlueGreenSwapTargetGroupsStep extends TaskExecutableWithRollbackAndRbac<EcsCommandResponse> {
    ...
    public static final String ECS_BLUE_GREEN_CREATE_SERVICE_STEP_MISSING = "Blue Green Create Service step is not configured.";
    ...
    public StepResponse handleTaskResultWithSecurityContext(Ambiance ambiance, StepElementParameters stepParameters,
        ...
        try {
            ...
            StepResponse.StepResponseBuilder stepResponseBuilder =
                    StepResponse.builder().unitProgressList(ecsBlueGreenSwapTargetGroupsResponse.getUnitProgressData().getUnitProgresses());

            stepResponse = generateStepResponse(ambiance, ecsBlueGreenSwapTargetGroupsResponse, stepResponseBuilder);
        } catch (Exception e) {
            log.error("Error while processing ecs blue green swap target groups response: {}", ExceptionUtils.getMessage(e), e);
            throw e;
        }
        return stepResponse;
    }
    ...
    private StepResponse generateStepResponse(
            Ambiance ambiance, EcsBlueGreenSwapTargetGroupsResponse ecsBlueGreenSwapTargetGroupsResponse,
            StepResponse.StepResponseBuilder stepResponseBuilder) {
        ...
        if (ecsBlueGreenSwapTargetGroupsResponse.getCommandExecutionStatus() != CommandExecutionStatus.SUCCESS) {
            stepResponse = stepResponseBuilder.status(Status.FAILED)
                    .failureInfo(FailureInfo.newBuilder()
                            .setErrorMessage(ecsStepCommonHelper.getErrorMessage(ecsBlueGreenSwapTargetGroupsResponse))
                            .build())
                    .build();
        } else {
            ...
            executionSweepingOutputService.consume(ambiance, OutcomeExpressionConstants.ECS_BLUE_GREEN_SWAP_TARGET_GROUPS_OUTCOME,
    ...
    public TaskRequest obtainTaskAfterRbac(Ambiance ambiance, StepElementParameters stepParameters, StepInputPackage inputPackage) {
        ...
        OptionalSweepingOutput ecsBlueGreenPrepareRollbackDataOptional = executionSweepingOutputService.resolveOptional(ambiance,
                RefObjectUtils.getSweepingOutputRefObject(ecsBlueGreenSwapTargetGroupsStepParameters.getEcsBlueGreenCreateServiceFnq() + "."
                        + OutcomeExpressionConstants.ECS_BLUE_GREEN_PREPARE_ROLLBACK_DATA_OUTCOME));

        OptionalSweepingOutput ecsBlueGreenCreateServiceDataOptional = executionSweepingOutputService.resolveOptional(ambiance,
                RefObjectUtils.getSweepingOutputRefObject(ecsBlueGreenSwapTargetGroupsStepParameters.getEcsBlueGreenCreateServiceFnq() + "."
