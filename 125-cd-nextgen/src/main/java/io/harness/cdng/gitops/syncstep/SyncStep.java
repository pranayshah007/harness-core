...
public class SyncStep implements SyncExecutableWithRbac<StepElementParameters> {
  ...
  public StepResponse executeSyncAfterRbac(Ambiance ambiance, StepElementParameters stepParameters,
    ...
    if (!cdFeatureFlagHelper.isEnabled(accountId, FeatureName.GITOPS_SYNC_STEP)) {
      throw new InvalidRequestException("Operation not supported.", USER);
    }

    String orgId = AmbianceUtils.getOrgIdentifier(ambiance);
    String projectId = AmbianceUtils.getProjectIdentifier(ambiance);

    final StepResponse.StepResponseBuilder stepResponseBuilder = StepResponse.builder();
  ...
  private ApplicationSyncRequest getSyncRequest(SyncStepParameters syncStepParameters, Application application,
    ...
    boolean syncAllResources = syncStepParameters.getSyncResources().getValue().equals(SyncResources.ALL.getValue());
    List<ApplicationResource.Resource> outOfSyncResources = null;
    if (!syncAllResources) {
      // get "OutOfSync" resources and call sync
      outOfSyncResources = SyncStepHelper.getOnlyOutOfSyncResources(resources);
    }

    String targetRevision = getTargetRevision(latestApplicationState);

    ApplicationSyncRequest syncRequest = SyncStepHelper.getSyncRequest(
        application, targetRevision, syncStepParameters, syncAllResources, outOfSyncResources);
    return syncRequest;
  }
  ...
  private boolean isAutoSyncEnabled(ApplicationResource.SyncPolicy syncPolicy) {
    return syncPolicy.getAutomated() != null ? true : false;
  }
