...
import io.harness.delegate.task.aws.asg.AsgRollingRollbackConfig;
...
public class AsgRollingRollbackStep extends TaskExecutableWithRollbackAndRbac<AsgCommandResponse> {
  ...
  public StepResponse handleTaskResultWithSecurityContext(Ambiance ambiance,
    ...
    try {
      ...
      stepResponse = generateStepResponse(ambiance, asgRollingRollbackResponse, stepResponseBuilder);
    } catch (Exception e) {
  ...
  private StepResponse generateStepResponse(Ambiance ambiance, AsgRollingRollbackResponse asgRollingRollbackResponse,
      StepResponseBuilder stepResponseBuilder) {
    ...
    if (asgRollingRollbackResponse.getCommandExecutionStatus() != CommandExecutionStatus.SUCCESS) {
      ...
      AsgRollingRollbackOutcome asgRollingRollbackOutcome =
          AsgRollingRollbackOutcome.builder().firstDeployment(asgRollingRollbackResult.isFirstDeployment()).build();

      executionSweepingOutputService.consume(ambiance, OutcomeExpressionConstants.ASG_ROLLING_ROLLBACK_OUTCOME,
          asgRollingRollbackOutcome, StepOutcomeGroup.STEP.name());
  ...
  public TaskRequest obtainTaskAfterRbac(
    ...
    if (EmptyPredicate.isEmpty(asgRollingRollbackStepParameters.getAsgRollingRollbackFqn())) {
    ...
    OptionalSweepingOutput asgRollingRollbackDataOptionalOutput =
        executionSweepingOutputService.resolveOptional(ambiance,
            RefObjectUtils.getSweepingOutputRefObject(asgRollingRollbackStepParameters.getAsgRollingRollbackFqn() + "."
                + OutcomeExpressionConstants.ASG_ROLLING_ROLLBACK_OUTCOME));

    if (!asgRollingRollbackDataOptionalOutput.isFound()) {
      return TaskRequest.newBuilder()
          .setSkipTaskRequest(SkipTaskRequest.newBuilder()
                                  .setMessage("Asg Rolling Deploy Step was not executed. Skipping Rollback.")
                                  .build())
    ...
    AsgRollingRollbackDataOutcome asgRollingRollbackDataOutcome =
        (AsgRollingRollbackDataOutcome) asgRollingRollbackDataOptionalOutput.getOutput();

    InfrastructureOutcome infrastructureOutcome = (InfrastructureOutcome) outcomeService.resolve(
        ambiance, RefObjectUtils.getOutcomeRefObject(OutcomeExpressionConstants.INFRASTRUCTURE_OUTCOME));

    AsgRollingRollbackConfig asgRollingRollbackConfig =
        AsgRollingRollbackConfig.builder()
            .asgName(asgRollingRollbackDataOutcome.getAsgName())
            .isFirstDeployment(asgRollingRollbackDataOutcome.isFirstDeployment())
            .asgStoreManifestsContent(asgRollingRollbackDataOutcome.getAsgStoreManifestsContent())
            .build();
