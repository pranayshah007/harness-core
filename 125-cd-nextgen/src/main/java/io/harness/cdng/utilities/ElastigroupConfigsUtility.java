...
@OwnedBy(CDP)
@UtilityClass
public class ElastigroupConfigsUtility {
...
//  }
  ...
  public Dependency getDependencyMetadata(final String elastigroupConfigPlanNodeId, StepParameters stepParameters,
      KryoSerializer kryoSerializer, String metadataKey) {
    Map<String, ByteString> metadataDependency =
        prepareMetadataForElastigroupConfigPlanCreator(elastigroupConfigPlanNodeId, stepParameters, kryoSerializer, metadataKey);
    return Dependency.newBuilder().putAllMetadata(metadataDependency).build();
  }

  private Map<String, ByteString> prepareMetadataForElastigroupConfigPlanCreator(
      String elastigroupConfigPlanNodeId, StepParameters stepParameters, KryoSerializer kryoSerializer, String metadataKey) {
    Map<String, ByteString> metadataDependency = new HashMap<>();
    metadataDependency.put(YamlTypes.UUID, ByteString.copyFrom(kryoSerializer.asDeflatedBytes(elastigroupConfigPlanNodeId)));
