...
import static io.harness.delegate.task.googlefunctions.request.GoogleFunctionDeployWithoutTrafficRequest.GoogleFunctionDeployWithoutTrafficRequestBuilder;
...
@NoArgsConstructor
@Slf4j
public class GoogleFunctionsDeployWithoutTrafficStep extends TaskChainExecutableWithRollbackAndRbac implements GoogleFunctionsStepExecutor {
    ...
    @Override
    public TaskChainResponse executeTask(Ambiance ambiance, StepElementParameters stepParameters, GoogleFunctionsStepPassThroughData googleFunctionsStepPassThroughData, UnitProgressData unitProgressData) {
        ...
        GoogleFunctionDeployWithoutTrafficRequestBuilder
                googleFunctionDeployWithoutTrafficRequestBuilder =
        ...
        return googleFunctionsHelper.queueTask(stepParameters, googleFunctionDeployWithoutTrafficRequestBuilder.build(), ambiance,
    ...
    public StepResponse finalizeExecutionWithSecurityContext(Ambiance ambiance, StepElementParameters stepParameters, PassThroughData passThroughData, ThrowingSupplier<ResponseData> responseDataSupplier) throws Exception {
        ...
        try {
            googleFunctionDeployWithoutTrafficResponse = (GoogleFunctionDeployWithoutTrafficResponse) responseDataSupplier.get();
        } catch (Exception e) {
        ...
        StepResponseBuilder stepResponseBuilder =
                StepResponse.builder().unitProgressList(googleFunctionDeployWithoutTrafficResponse.getUnitProgressData().getUnitProgresses());
        if (googleFunctionDeployWithoutTrafficResponse.getCommandExecutionStatus() != CommandExecutionStatus.SUCCESS) {
            return GoogleFunctionsHelper.getFailureResponseBuilder(googleFunctionDeployWithoutTrafficResponse, stepResponseBuilder)
                    .build();
        }
        GoogleFunctionStepOutcome googleFunctionDeployOutcome = googleFunctionsHelper.getGoogleFunctionStepOutcome(
                googleFunctionDeployWithoutTrafficResponse.getFunction());
        executionSweepingOutputService.consume(ambiance, OutcomeExpressionConstants.GOOGLE_FUNCTION_DEPLOY_WITHOUT_TRAFFIC_OUTCOME,
                googleFunctionDeployOutcome, StepOutcomeGroup.STEP.name());
