...
public class RetrySGAdviserWithRollback implements Adviser {
  ...
  @Inject
  OnFailPipelineRollbackAdviser onFailPipelineRollbackAdviser;
  ...
  public AdviserResponse onAdviseEvent(AdvisingEvent advisingEvent) {
    ...
    Optional<Level> optSGLevel = AmbianceUtils.getClosestStepGroupLevelFromAmbiance(advisingEvent.getAmbiance());
    if (optSGLevel.isEmpty()) {
      return null;
    }
    int retryIndex = optSGLevel.get().getRetryIndex();
    if (retryIndex >= parameters.getRetryCount()) {
