...
public class ResourceGroupServiceImplTest extends ResourceGroupTestBase {
  ...
  public void setup() {
    ...
    ResourceGroup.ResourceGroupBuilder resourceGroupBuilder = ResourceGroup.builder()
                                                                  .accountIdentifier(ACCOUNT_IDENTIFIER)
                                                                  .orgIdentifier(ORG_IDENTIFIER)
                                                                  .projectIdentifier(PROJECT_IDENTIFIER);

    resourceGroupList.add(resourceGroupBuilder.identifier("RG1").build());
    resourceGroupList.add(resourceGroupBuilder.identifier("RG2").build());
    resourceGroupList.add(resourceGroupBuilder.identifier("RG3").build());
    ...
    AccessControlDTO.AccessControlDTOBuilder accessControlDTOBuilder =
  ...
  public void testListFilter_withSelectedViewPermission_withNoIdentifierFilter() {
    ...
    when(resourceGroupV2RepositoryMock.findAll(criteria, getPageRequest(pageRequest)))
    ...
    resourceGroupServiceMockRepo.list(resourceGroupFilterDTO, pageRequest);
    verify(resourceGroupServiceMockRepo, times(2))
  ...
  public void testListFilter_withSelectedViewPermission_withIdentifierFilter() {
    ...
    resourceGroupFilterDTO.setIdentifierFilter(
        permittedResourceGroups.stream().map(ResourceGroup::getIdentifier).collect(Collectors.toSet()));

    final Criteria resourceGroupFilterCriteria2 =
        resourceGroupServiceMockRepo.getResourceGroupFilterCriteria(resourceGroupFilterDTO);

    verify(resourceGroupV2RepositoryMock, times(1)).findAll(resourceGroupFilterCriteria, Pageable.unpaged());
    verify(resourceGroupV2RepositoryMock, times(1)).findAll(resourceGroupFilterCriteria2, getPageRequest(pageRequest));
  }
}
