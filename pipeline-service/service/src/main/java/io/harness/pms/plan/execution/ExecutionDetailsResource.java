...
public class ExecutionDetailsResource {
  ...
  @Operation(operationId = "IDP Plugin -getListOfExecutionsForMultiplePipelinesIdentifiers",
      description = "IDP Plugin -Returns a List of Pipeline Executions with Specific Filters",
      summary = "IDP Plugin -List Executions",
      responses =
      {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "default",
            description = "IDP Plugin -Returns all the Executions of pipelines for given filters")
      })
  @NGAccessControlCheck(resourceType = "PIPELINE", permission = PipelineRbacPermissions.PIPELINE_VIEW)
  public ResponseDTO<Page<PipelineExecutionSummaryDTO>>
  getListOfExecutionsIDPPluginSupport(
          ...
          content =
          {
            @Content(mediaType = "application/json",
                examples = @ExampleObject(name = "List", summary = "Sample List Pipeline Executions",
                    value = PipelineAPIConstants.LIST_EXECUTIONS,
                    description = "Sample List Pipeline Executions JSON Payload"))
          }) PipelineExecutionFilterPropertiesIDPPluginSupportDTO filterProperties,
      @QueryParam("status") List<ExecutionStatus> statusesList, @QueryParam("myDeployments") boolean myDeployments,
      @BeanParam GitEntityFindInfoDTO gitEntityBasicInfo) {
    log.info("Get List of executions");
    ByteString gitSyncBranchContext = pmsGitSyncHelper.getGitSyncBranchContextBytesThreadLocal();
    ...
    Criteria criteria = pmsExecutionService.formCriteriaIDPPluginSupport(accountId, orgId, projectId,
        pipelineIdentifier, filterIdentifier, filterProperties, moduleName, searchTerm, statusesList, myDeployments,
        false, gitSyncBranchContext, true);
    Pageable pageRequest;
    if (EmptyPredicate.isEmpty(sort)) {
      pageRequest = PageRequest.of(page, size, Sort.by(Direction.DESC, PipelineEntityKeys.createdAt));
    } else {
