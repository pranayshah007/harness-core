...
import io.harness.steps.barriers.event.BarrierWithinStrategyInitializer;
...
public class PipelineServiceApplication extends Application<PipelineServiceConfiguration> {
  ...
  private static void registerObservers(PipelineServiceConfiguration appConfig, Injector injector) {
    ...
    SpawnChildrenRequestProcessor spawnChildrenRequestProcessor = injector.getInstance(Key.get(SpawnChildrenRequestProcessor.class));
    spawnChildrenRequestProcessor.getBarrierInitializer().register(injector.getInstance(Key.get(BarrierWithinStrategyInitializer.class)));
