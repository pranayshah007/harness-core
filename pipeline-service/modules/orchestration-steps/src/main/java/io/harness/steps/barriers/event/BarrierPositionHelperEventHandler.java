...
public class BarrierPositionHelperEventHandler implements AsyncInformObserver, NodeStatusUpdateObserver {
  ...
  private List<BarrierExecutionInstance> updatePosition(
    ...
    return barrierService.updatePosition(planExecutionId, type, level.getSetupId(), nodeExecution.getUuid(), stageLevel.isPresent() ? stageLevel.get().getRuntimeId() : null, stepGroupLevel.<String>map(Level::getRuntimeId).orElse(null));
  }
