...
public class IssueCommentTriggerFilterTest extends CategoryTest {
  private Logger logger;
  private ListAppender<ILoggingEvent> listAppender;
  @Inject @InjectMocks private IssueCommentTriggerFilter issueCommentTriggerFilter;
  ...
  public void applyFilterTest() throws IOException {
    ...
    WebhookEventMappingResponse webhookEventMappingResponse = issueCommentTriggerFilter.applyFilter(filterRequestData);
  ...
  public void applyFilterExceptionTest() throws IOException {
    ...
    String prJson = Resources.toString(testFile, Charsets.UTF_8);
    doReturn(BinaryResponseData.builder().data(data).build()).when(taskExecutionUtils).executeSyncTask(any());
    doThrow(InvalidRequestException.class).when(webhookParserSCMService).convertPRWebhookEvent(any());
    doReturn(GitApiTaskResponse.builder()
                 .commandExecutionStatus(CommandExecutionStatus.SUCCESS)
                 .gitApiResult(GitApiFindPRTaskResponse.builder().prJson(prJson).build())
                 .build())
    ...
    assertThat(webhookEventMappingResponse.getWebhookEventResponse().getMessage())
        .isEqualTo("Failed to update Webhook payload with PR Details: io.harness.exception.InvalidRequestException");
