...
//    Instant endTime = Instant.ofEpochMilli(jobConstants.getJobEndTime());
    ...
    //       {folder_to_ingest: [list of CSVs]} - it will have CSVs across months and across different connectors/awsaccounts
    ...
    for (S3ObjectSummary objectSummary : s3Objects) {
      ...
      if (path.size() != 5 && path.size() != 6) continue;
      ...
      if (objectSummary.getLastModified().compareTo(java.util.Date.from(startTime)) >= 0 &&
        ...
        if ( objectSummary.getKey().endsWith(".csv.gz") || objectSummary.getKey().endsWith(".csv.zip") || objectSummary.getKey().endsWith(".csv") ) {
    ...
    for (String folderPath : foldersToIngest) {
      ...
      clickHouseService.executeClickHouseQuery("DROP TABLE IF EXISTS " + awsBillingTableId, Boolean.FALSE);


      String createAwsBillingTableQuery = "CREATE TABLE IF NOT EXISTS " + awsBillingTableId + " (" + schema + " ) ENGINE = MergeTree ORDER BY tuple(usagestartdate) SETTINGS allow_nullable_key = 1;";
      // System.out.println(createAwsBillingTableQuery);
      log.info(createAwsBillingTableQuery);
      clickHouseService.executeClickHouseQuery(createAwsBillingTableQuery, Boolean.FALSE);
  ...
  public void ingestDataIntoCostAgg(List<String> usageAccountIds, String month) throws Exception {
    ...
    String deleteQuery = "DELETE from ccm.costAggregated WHERE DATE_TRUNC('month', day) = DATE_TRUNC('month', toDateTime('" +
            month + " 00:00:00')) AND cloudProvider = 'AWS';";
    ...
    String insertQuery = "INSERT INTO ccm.costAggregated (day, cost, cloudProvider)  " +
            ...
            "  WHERE DATE_TRUNC('month', day) = DATE_TRUNC('month', toDateTime('" +
            month + " 00:00:00')) AND cloudProvider = 'AWS'" +
            "  GROUP BY day;";
...
//             "        arrayFilter(x -> isNotNull(x), arrayMap(i -> if((tagsPresent[i]) = 0, toString(NULL), tagsAllKey[i]), arrayEnumerate(tagsPresent))) AS tagsKey, " +
  ...
  public List<String> getUniqueAccountIds(String awsBillingTableId) throws Exception {
    ...
    return clickHouseService.executeClickHouseQuery(selectQuery, Boolean.TRUE);
  }


  public void insertIntoAwsBillingTableFromS3Bucket(String awsBillingTableId, String csvFolderPath) throws Exception {
    String insertQuery = "SET input_format_csv_skip_first_lines=1; INSERT INTO " + awsBillingTableId + " SELECT * FROM s3('https://" + configuration.getAwsS3SyncConfig().getAwsS3BucketName() +
            ".s3.amazonaws.com/" + csvFolderPath + "/*.csv.gz','" + configuration.getAwsS3SyncConfig().getAwsAccessKey() + "','" + configuration.getAwsS3SyncConfig().getAwsSecretKey() +
  ...
  public String fetchSchemaFromManifestFileInFolder(String folderPath) throws Exception {
    AWSCredentialsProvider credentials = awsClient.constructStaticBasicAwsCredentials(configuration.getAwsS3SyncConfig().getAwsAccessKey(), configuration.getAwsS3SyncConfig().getAwsSecretKey());
    S3Objects s3Objects = awsClient.getIterableS3ObjectSummaries(credentials, configuration.getAwsS3SyncConfig().getAwsS3BucketName(), folderPath);
    for (S3ObjectSummary objectSummary : s3Objects) {
  ...
  public boolean isValidMonthFolder(String folderName) throws Exception {
    try {
      ...
      if (startStr.length() != 8 || endStr.length() != 8 || Integer.parseInt(endStr) <= Integer.parseInt(startStr)) return Boolean.FALSE;
  ...
  public void createDBAndTables() throws Exception {
    clickHouseService.executeClickHouseQuery(createCCMDBQuery, Boolean.FALSE);
    clickHouseService.executeClickHouseQuery(createAwsCurTableQuery, Boolean.FALSE);
    clickHouseService.executeClickHouseQuery(createUnifiedTableTableQuery, Boolean.FALSE);
    clickHouseService.executeClickHouseQuery(createPreAggregatedTableQuery, Boolean.FALSE);
    clickHouseService.executeClickHouseQuery(createCostAggregatedTableQuery, Boolean.FALSE);
  }
