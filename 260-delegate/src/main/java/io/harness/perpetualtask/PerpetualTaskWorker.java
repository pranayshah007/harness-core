...
public class PerpetualTaskWorker {
  ...
  @Inject
  public PerpetualTaskWorker(PerpetualTaskServiceClient perpetualTaskServiceClient,
  ...
  List<PerpetualTaskAssignDetails> fetchAssignedTask() {
    ...
    List<PerpetualTaskAssignDetails> assignedTasks = perpetualTaskServiceClient.perpetualTaskList(delegateId);
  ...
  void startTask(PerpetualTaskAssignDetails task) {
    try (AutoLogContext ignore1 = new PerpetualTaskLogContext(task.getTaskId().getId(), OVERRIDE_ERROR)) {
      PerpetualTaskExecutionContext context = perpetualTaskServiceClient.perpetualTaskContext(task.getTaskId());
      ...
      PerpetualTaskLifecycleManager perpetualTaskLifecycleManager =
          new PerpetualTaskLifecycleManager(task.getTaskId(), context, factoryMap, perpetualTaskServiceClient,
