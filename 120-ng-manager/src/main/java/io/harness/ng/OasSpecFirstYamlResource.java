...
public class OasSpecFirstYamlResource {
  ...
  public Response getOpenApiYamlNGManager() {
    try {
      String yaml = String.valueOf(new File(NG_MANAGER_PATH));
      return Response.ok(yaml, "application/yaml")
  ...
  public Response getOpenApiYamlConnectors() {
    try {
      String yaml = String.valueOf(new File(CONNECTORS_PATH));
      return Response.ok(yaml, "application/yaml")
          .header("Content-Disposition", "attachment; filename=\"openapi.yaml\"")
          .build();
    } catch (Exception e) {
  ...
  public Response getOpenApiYamlCommons() {
    try {
      String yaml = String.valueOf(new File(COMMONS_PATH));
      return Response.ok(yaml, "application/yaml")
          .header("Content-Disposition", "attachment; filename=\"openapi.yaml\"")
          .build();
    } catch (Exception e) {
