...
public class ProjectFavoriteApiImpl implements ProjectFavoritesApi {
  ...
  @Override
  public Response createProjectFavorite(String org, String project, @NotNull String userId, String harnessAccount,
      String resourceType, String resourceId) {
    return Response.status(Response.Status.CREATED)
        .entity(favoriteService.createFavorite(
            harnessAccount, org, project, userId, ResourceType.valueOf(resourceType), resourceId))
        .build();
  }
  ...
  public Response getProjectFavorite(
    ...
    if (isNotEmpty(resourceType)) {
      return Response.ok()
          .entity(favoriteService.getFavorite(harnessAccount, org, project, userId, ResourceType.valueOf(resourceType)))
          .build();
    }
    return Response.ok().entity(favoriteService.getAllFavoritesOfUser(harnessAccount, org, project, userId)).build();
  }
}
