...
then
    ...
    java -jar bazel-bin/pipeline-service/module_deploy.jar generate-openapi-spec target/800_target.json || PIPELINE_SERVICE_T=$?
fi
...
then
    ...
    java -jar bazel-bin/pipeline-service/module_deploy.jar generate-openapi-spec target/800_source.json || PIPELINE_SERVICE_S=$?
fi
