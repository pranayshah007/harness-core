...
CI_MANAGER_T=0
bazel build ${BAZEL_ARGS} -- //ci-manager:module_deploy.jar || CI_MANAGER_T=$?
...
then
    ...
    java -jar bazel-bin/ci-manager/module_deploy.jar generate-openapi-spec target/310_target.json  ci-manager/config/ci-manager-config.yml || CI_MANAGER_T=$?
fi
...
then
    echo "BUILD CI_MANAGER"
    CI_MANAGER_S=0
    bazel build ${BAZEL_ARGS} -- //ci-manager:module_deploy.jar || CI_MANAGER_S=$?
else
    CI_MANAGER_S=1
fi
...
then
    ...
    java -jar bazel-bin/ci-manager/module_deploy.jar generate-openapi-spec target/310_source.json ci-manager/config/ci-manager-config.yml || CI_MANAGER_S=$?
fi
