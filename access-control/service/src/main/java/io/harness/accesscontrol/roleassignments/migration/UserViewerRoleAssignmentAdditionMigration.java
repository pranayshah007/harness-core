...
public class UserViewerRoleAssignmentAdditionMigration implements NGMigration {
  ...
  @Inject
  public UserViewerRoleAssignmentAdditionMigration(
      RoleAssignmentRepository roleAssignmentRepository, UserRepository userRepository, ScopeService scopeService) {
          ...
          for (UserDBO userDBO : userDBOList) {
            try {
              ...
              if (!isEmpty(scopeParams.getProjectIdentifier())) {
                roleAssignmentDBO = buildRoleAssignmentDBO(PROJECT_VIEWER_ROLE,
                    DEFAULT_PROJECT_LEVEL_RESOURCE_GROUP_IDENTIFIER, userDBO.getIdentifier(), scopeParams);
              } else if (!isEmpty(scopeParams.getOrgIdentifier())) {
                roleAssignmentDBO = buildRoleAssignmentDBO(ORGANIZATION_VIEWER_ROLE,
                    DEFAULT_ORGANIZATION_LEVEL_RESOURCE_GROUP_IDENTIFIER, userDBO.getIdentifier(), scopeParams);
              } else {
                roleAssignmentDBO = buildRoleAssignmentDBO(ACCOUNT_VIEWER_ROLE,
                    DEFAULT_ACCOUNT_LEVEL_RESOURCE_GROUP_IDENTIFIER, userDBO.getIdentifier(), scopeParams);
              }
