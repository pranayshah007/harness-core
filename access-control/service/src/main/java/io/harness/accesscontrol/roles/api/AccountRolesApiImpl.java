...
public class AccountRolesApiImpl implements AccountRolesApi {
  ...
  public Response createRoleAcc(CreateRoleRequest body, @AccountIdentifier String account) {
    ...
    RoleDTO roleDTO = rolesApiUtils.getRoleAccDTO(body);
  ...
  public Response deleteRoleAcc(String role, String account) {
    accessControlClient.checkForAccessOrThrow(
        ResourceScope.of(account, null, null), Resource.of(ROLE, role), DELETE_ROLE_PERMISSION);
    HarnessScopeParams harnessScopeParams = HarnessScopeParams.builder().accountIdentifier(account).build();
  ...
  public Response getRoleAcc(String role, String account) {
    accessControlClient.checkForAccessOrThrow(
        ResourceScope.of(account, null, null), Resource.of(ROLE, role), VIEW_ROLE_PERMISSION);
    HarnessScopeParams harnessScopeParams = HarnessScopeParams.builder().accountIdentifier(account).build();
    String scopeIdentifier = ScopeMapper.fromParams(harnessScopeParams).toString();
    RolesResponse response = RolesApiUtils.getRolesResponse(roleDTOMapper.toResponseDTO(
        roleService.get(role, scopeIdentifier, NO_FILTER).<InvalidRequestException>orElseThrow(() -> {
          throw new InvalidRequestException("Role not found in Account scope for given identifier.");
        })));
    return Response.ok().entity(response).build();
  }
  ...
  public Response listRolesAcc(
    ...
    accessControlClient.checkForAccessOrThrow(
        ResourceScope.of(account, null, null), Resource.of(ROLE, null), VIEW_ROLE_PERMISSION);
    HarnessScopeParams harnessScopeParams = HarnessScopeParams.builder().accountIdentifier(account).build();
    String scopeIdentifier = ScopeMapper.fromParams(harnessScopeParams).toString();
  ...
  @Override
  public Response updateRoleAcc(CreateRoleRequest body, String role, String account) {
    accessControlClient.checkForAccessOrThrow(
    ...
    String scopeIdentifier = ScopeMapper.fromParams(harnessScopeParams).toString();
    Role updatedRole = roleService.update(fromDTO(scopeIdentifier, rolesApiUtils.getRoleAccDTO(body)));
    RoleResponseDTO responseDTO = roleDTOMapper.toResponseDTO(updatedRole);
    RolesResponse response = RolesApiUtils.getRolesResponse(responseDTO);
    return Response.ok().entity(response).build();
  }
}
