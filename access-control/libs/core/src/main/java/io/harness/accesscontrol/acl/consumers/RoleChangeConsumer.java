...
public class RoleChangeConsumer implements AccessControlChangeConsumer<RoleChangeEventData> {
    ...
    public RoleChangeConsumer(ACLRepository aclRepository, RoleAssignmentRepository roleAssignmentRepository, RoleRepository roleRepository,
    ...
    public boolean consumeUpdateEvent(String id, RoleChangeEventData roleChangeEventData) {
        ...
        Criteria criteria = Criteria.where(RoleAssignmentDBO.RoleAssignmentDBOKeys.roleIdentifier).is(updatedRole.getIdentifier());
        if (!StringUtils.isEmpty(updatedRole.getScopeIdentifier())) {
            criteria.and(RoleAssignmentDBO.RoleAssignmentDBOKeys.scopeIdentifier).is(updatedRole.getScopeIdentifier());
        }
        ...
        try (DelayLogContext ignore = new DelayLogContext(permissionsChangeTime, OVERRIDE_ERROR)) {
            log.info(
                    ...
                    numberOfACLsCreated, updatedRole.getIdentifier(), updatedRole.getScopeIdentifier(),
                    permissionsChangeTime);
            log.info(
                    "RoleChangeConsumer.consumeUpdateEvent: Number of ACLs deleted: {} for identifier: {}, scope: {} Time taken: {}",
                    numberOfACLsDeleted, updatedRole.getIdentifier(), updatedRole.getScopeIdentifier(),
                    permissionsChangeTime);
        }
        return true;
    }
    ...
    private static class ReProcessRoleAssignmentOnRoleUpdateTask implements Callable<Result> {
        ...
        private ReProcessRoleAssignmentOnRoleUpdateTask(ACLRepository aclRepository,
                                                        ACLGeneratorService aclGeneratorService, RoleAssignmentDBO roleAssignment, RoleChangeEventData roleChangeEventData) {
            this.aclRepository = aclRepository;
            this.aclGeneratorService = aclGeneratorService;
        ...
        public Result call() {
            ...
            long numberOfACLsDeleted = aclRepository.deleteByRoleAssignmentIdAndPermissions(roleAssignmentDBO.getId(), permissionsRemoved);
            long numberOfACLsCreated = 0;
