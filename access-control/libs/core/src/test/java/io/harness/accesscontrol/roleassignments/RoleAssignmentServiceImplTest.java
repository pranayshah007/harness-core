...
public class RoleAssignmentServiceImplTest extends AccessControlCoreTestBase {
  ...
  public void testCreate() {
    ...
    ArgumentCaptor<RoleAssignmentCreateEventV2> argumentCaptor = ArgumentCaptor.forClass(RoleAssignmentCreateEventV2.class);
    ...
    when(outboxTransactionTemplate.execute(any()))
        .thenAnswer(invocationOnMock
            -> invocationOnMock.getArgument(0, TransactionCallback.class)
                   .doInTransaction(new SimpleTransactionStatus()));
    when(outboxService.save(any())).thenReturn(null);
    ArgumentCaptor<RoleAssignmentUpdateEventV2> argumentCaptor = ArgumentCaptor.forClass(RoleAssignmentUpdateEventV2.class);
