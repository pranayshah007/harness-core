...
public class CIExecutionPoller implements Managed {
    @Inject CIManagerConfiguration ciManagerConfiguration;
    ...
    private void processMessage(DequeueResponse message) {
        ...
        String authToken = ciManagerConfiguration.getQueueServiceClient().getAuthToken();
        if (ciInitTaskMessageProcessor.processMessage(message)) {
            hsqsServiceClient.ack(
                    AckRequest.builder().itemID(message.getItemId()).topic(moduleName).build(), authToken);
        } else {
            hsqsServiceClient.unack(
                    UnAckRequest.builder().itemID(message.getItemId()).topic(moduleName).build(), authToken);
        }
    }
