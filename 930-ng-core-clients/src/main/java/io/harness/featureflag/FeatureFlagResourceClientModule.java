...
public class FeatureFlagResourceClientModule extends AbstractModule {
  ...
  @Provides
  @Singleton
  private FeatureFlagResourceClientHttpFactory secretManagerHttpClientFactory(
      KryoConverterFactory kryoConverterFactory) {
    return new FeatureFlagResourceClientHttpFactory(this.ngManagerClientConfig, this.serviceSecret,
        new ServiceTokenGenerator(), kryoConverterFactory, clientId, clientMode);
  }

  @Override
  protected void configure() {
    this.bind(FeatureFlagResourceClient.class)
        .toProvider(FeatureFlagResourceClientHttpFactory.class)
        .in(Scopes.SINGLETON);
  }
}
