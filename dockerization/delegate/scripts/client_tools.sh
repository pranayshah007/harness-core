...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && curl -s -L -o client-tools/go-template/v0.4.1/go-template https://app.harness.io/public/shared/tools/go-template/release/v0.4.1/bin/linux/$ARCH/go-template \
  ...
  && mkdir -m 777 -p client-tools/scm/4680eeef \
  && curl -s -L -o client-tools/scm/4680eeef/scm https://storage.googleapis.com/harness-qa-public/public/shared/tools/scm/release/4680eeef/bin/linux/$ARCH/scm
else
