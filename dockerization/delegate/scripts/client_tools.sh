...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/aeda065c \
  && curl -s -L -o client-tools/scm/aeda065c/scm https://app.harness.io/public/shared/tools/scm/release/aeda065c/bin/linux/amd64/scm
else
