...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/d6b5dabd \
  && curl -s -L -o client-tools/scm/d6b5dabd/scm https://app.harness.io/public/shared/tools/scm/release/d6b5dabd/bin/linux/$ARCH/scm
else
