...
if [[ ( -z "$DISABLE_CLIENT_TOOLS") || ("$DISABLE_CLIENT_TOOLS" = "false") ]]; then
  ...
  && mkdir -m 777 -p client-tools/scm/df0e7d04 \
  && curl -s -L -o client-tools/scm/df0e7d04/scm https://app.harness.io/public/shared/tools/scm/release/df0e7d04/bin/linux/$ARCH/scm
else
