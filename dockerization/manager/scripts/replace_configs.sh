...
if [[ "" != "$REDIS_URL" ]]; then
  ...
  export REDIS_URL; yq -i '.redisConfig.redisUrl=env(REDIS_URL)' $CONFIG_FILE
...
if [[ "$REDIS_SENTINEL" == "true" ]]; then
  ...
  yq -i '.redisConfig.sentinel=true' $CONFIG_FILE
...
if [[ "" != "$REDIS_MASTER_NAME" ]]; then
  ...
  export REDIS_MASTER_NAME; yq -i '.redisConfig.masterName=env(REDIS_MASTER_NAME)' $CONFIG_FILE
...
if [[ "" != "$REDIS_SENTINELS" ]]; then
  ...
  for REDIS_SENTINEL_URL in "${REDIS_SENTINEL_URLS[@]}"; do
    export REDIS_SENTINEL_URL; export INDEX; yq -i '.redisLockConfig.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
     export REDIS_SENTINEL_URL; export INDEX; yq -i '.redisConfig.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
...
if [[ "" != "$REDIS_ENV_NAMESPACE" ]]; then
    ...
    export REDIS_ENV_NAMESPACE; yq -i '.redisConfig.envNamespace=env(REDIS_ENV_NAMESPACE)' $CONFIG_FILE
...
if [[ "" != "$REDIS_CONNECTION_POOL_SIZE" ]]; then
  ...
  export REDIS_CONNECTION_POOL_SIZE; yq -i '.redisConfig.connectionPoolSize=env(REDIS_CONNECTION_POOL_SIZE)' $CONFIG_FILE
...
if [[ "" != "$REDIS_RETRY_INTERVAL" ]]; then
  ...
  export REDIS_RETRY_INTERVAL; yq -i '.redisConfig.retryInterval=env(REDIS_RETRY_INTERVAL)' $CONFIG_FILE
...
if [[ "" != "$REDIS_RETRY_ATTEMPTS" ]]; then
  ...
  export REDIS_RETRY_ATTEMPTS; yq -i '.redisConfig.retryAttempts=env(REDIS_RETRY_ATTEMPTS)' $CONFIG_FILE
...
if [[ "" != "$REDIS_TIMEOUT" ]]; then
  ...
  export REDIS_TIMEOUT; yq -i '.redisConfig.timeout=env(REDIS_TIMEOUT)' $CONFIG_FILE
...
if [[ "$REDIS_SCRIPT_CACHE" == "false" ]]; then
  ...
  yq -i '.redisConfig.useScriptCache=false' $CONFIG_FILE
