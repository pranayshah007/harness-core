...
if [[ "" != "$MONGO_URI" ]]; then
  export MONGO_URI; yq -i '.cg.mongo.uri=env(MONGO_URI)' $CONFIG_FILE
fi
...
if [[ "" != "$MONGO_LOCK_URI" ]]; then
  export MONGO_LOCK_URI; yq -i '.cg.mongo.locksUri=env(MONGO_LOCK_URI)' $CONFIG_FILE
fi
...
if [[ "" != "$REDIS_SENTINELS" ]]; then
  ...
  for REDIS_SENTINEL_URL in "${REDIS_SENTINEL_URLS[@]}"; do
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.cg.redisLockConfig.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.cg.redisAtmosphereConfig.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
...
if [[ "" != "$EVENTS_FRAMEWORK_REDIS_SENTINELS" ]]; then
  ...
  for REDIS_SENTINEL_URL in "${SENTINEL_URLS[@]}"; do
    export REDIS_SENTINEL_URL; export INDEX; export INDEX; yq -i '.cg.eventsFramework.redis.sentinelUrls.[env(INDEX)]=env(REDIS_SENTINEL_URL)' $CONFIG_FILE
    INDEX=$(expr $INDEX + 1)
  done
fi
...
if [[ "" != "$SECOPS_EMAIL" ]]; then
 export SECOPS_EMAIL; yq -i '.cg.totp.secOpsEmail=env(SECOPS_EMAIL)' config.yml
fi

if [[ "" != "$INCORRECT_ATTEMPTS_UNTIL_SECOPS_NOTIFIED" ]]; then
 export INCORRECT_ATTEMPTS_UNTIL_SECOPS_NOTIFIED; yq -i '.cg.totp.incorrectAttemptsUntilSecOpsNotified=env(INCORRECT_ATTEMPTS_UNTIL_SECOPS_NOTIFIED)' config.yml
fi
