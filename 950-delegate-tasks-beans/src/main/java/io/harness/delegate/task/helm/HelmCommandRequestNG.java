      ...
      if (HELM_CHART == manifestDelegateConfig.getManifestType()) {
        switch (helManifestConfig.getStoreDelegateConfig().getType()) {
          case HTTP_HELM:
            ...
            populateDelegateSelectorCapability(capabilities,
                httpHelmStoreConfig.getHttpHelmConnector().getDelegateSelectors(),
                httpHelmStoreConfig.getHttpHelmConnector().getConnectorType());
            break;
