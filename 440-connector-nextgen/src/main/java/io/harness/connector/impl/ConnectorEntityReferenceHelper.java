...
public class ConnectorEntityReferenceHelper {
  ...
  public void sendSecretUsageEventForConnectorTest(String accountIdentifier, ConnectorInfoDTO connectorInfoDTO) {
    List<DecryptableEntity> decryptableEntities = connectorInfoDTO.getConnectorConfig().getDecryptableEntities();
    if (isEmpty(decryptableEntities)) {
      return;
    }
    ...
    secrets.forEach((key, value) -> {
      if (isNotEmpty(value.getIdentifier())) { // check for a better condition (eg- cert does not have any identifiers)
        EntityActivityCreateDTO ngActivityDTO =
            createTestConnectionActivity(accountIdentifier, connectorInfoDTO, value);
        try {
  ...
  private EntityActivityCreateDTO createTestConnectionActivity(
      String accountIdentifier, ConnectorInfoDTO connectorInfoDTO, SecretRefData secretRef) {
