...
public class AssignmentJsonValidator implements Validator<String> {
  ...
  public void validate(final String assignJson) {
    if (isBlank(assignJson)) {
      throw new AzureBPDeploymentException(ASSIGN_JSON_FILE_BLANK_VALIDATION_MSG);
    }

    Assignment assignment = JsonUtils.asObject(assignJson, Assignment.class);

    if (assignment.getIdentity() == null) {
      throw new AzureBPDeploymentException(AzureConstants.ASSIGNMENT_IDENTITY_NULL_VALIDATION_MSG);
    }
    if (isBlank(assignment.getLocation())) {
      throw new AzureBPDeploymentException(AzureConstants.ASSIGNMENT_LOCATION_BLANK_VALIDATION_MSG);
    }
    ...
    if (isBlank(blueprintId)) {
      throw new AzureBPDeploymentException(AzureConstants.PROPERTIES_BLUEPRINT_ID_VALIDATION_MSG);
    }

    if (!AzureResourceUtility.isValidResourceScope(blueprintId)) {
      throw new AzureBPDeploymentException(format(BLUEPRINT_ID_IS_NOT_VALIDATION_MSG, blueprintId));
    }
    ...
    if (!BLUEPRINT_ID_REGEX.matcher(blueprintId).matches()) {
      throw new AzureBPDeploymentException(format("Not valid value of properties.blueprintId property. Required format "
              + "/{resourceScope}/providers/Microsoft.Blueprint/blueprints/{blueprintName}/versions/{versionId}, but found - blueprintId: %s",
          blueprintId));
    }
    ...
    if (resourceScopeType == null) {
      throw new AzureBPDeploymentException(
  ...
  private void validateAssignmentPropertiesScope(Assignment assignment, ResourceScopeType resourceScopeType) {
    if (ResourceScopeType.MANAGEMENT_GROUP == resourceScopeType) {
      ...
      if (isBlank(scope)) {
        throw new AzureBPDeploymentException(AzureConstants.PROPERTIES_SCOPE_BLANK_VALIDATION_MSG);
      }
      if (!scope.startsWith(RESOURCE_SCOPE_SUBSCRIPTION_PATTERN)) {
        throw new AzureBPDeploymentException(format("Not valid value of properties.scope property. Required format "
