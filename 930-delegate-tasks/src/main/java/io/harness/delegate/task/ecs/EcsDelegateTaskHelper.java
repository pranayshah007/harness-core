...
public class EcsDelegateTaskHelper {
  ...
  public EcsCommandResponse getEcsCommandResponse(
    ...
    try {
      EcsCommandResponse ecsCommandResponse = commandTaskHandler.executeTask(
              ecsCommandRequest, iLogStreamingTaskClient, commandUnitsProgress);
      ecsCommandResponse.setCommandUnitsProgress(
      ...
      log.error("Exception in processing ecs task [{}]",
              ecsCommandRequest.getCommandName() + ":" + ecsCommandRequest.getEcsCommandType(),
              sanitizedException);
