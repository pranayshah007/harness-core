...
public class CfDeployCommandTaskHandlerNG extends CfCommandTaskNGHandler{
    ...
    protected CfCommandResponseNG executeTaskInternal(CfCommandRequestNG cfCommandRequestNG,
        ...
        if (!(cfCommandRequestNG instanceof CfDeployCommandRequestNG)) {
            throw new InvalidArgumentsException(Pair.of("cfCommandRequestNG", "Must be instance of CfDeployCommandRequestNG"));
        }
          ...
          if (workingDirectory == null) {
            ...
            if (DOWNSIZE_OLD_FIRST == cfDeployCommandRequestNG.getResizeStrategy()) {
                ...
                executionLogCallback = tasTaskHelperBase.getLogCallback(
                        iLogStreamingTaskClient, Upsize, true, commandUnitsProgress);
                cfCommandTaskHelperNG.upsizeNewApplication(executionLogCallback, cfDeployCommandRequestNG, cfServiceDataUpdated, cfRequestConfig, details,
                        cfInstanceElementsForVerification, cfAppAutoscalarRequestData);
    ...
    private void logException(
        ...
        log.error(CLOUD_FOUNDRY_LOG_PREFIX + "Exception in processing CF Deploy task [{}]", cfDeployCommandRequestNG,
                exception);
    ...
    void generateCfInstancesElementsForExistingApp(List<CfInternalInstanceElement> pcfInstanceElementsForVerification,
        ...
        try {
            ...
            executionLogCallback.saveExecutionLog(
                    "# Failed to fetch InstanceDetails for existing Application: " +
