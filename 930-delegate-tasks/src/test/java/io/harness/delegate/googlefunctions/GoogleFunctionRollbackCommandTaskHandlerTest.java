...
public class GoogleFunctionRollbackCommandTaskHandlerTest extends CategoryTest {
  ...
  public void executeTaskInternalSecondDeploymentTest() throws Exception {
    ...
    GoogleFunction googleFunction = GoogleFunction.builder()
                                        ...
                                        .build();
    doNothing().when(googleFunctionCommandTaskHelper).parseStringContentAsClassBuilder(anyString(), any(), anyString());
    doNothing()
    ...
    doReturn(Optional.of(function))
        .when(googleFunctionCommandTaskHelper)
        .getFunction(eq(""), isA(GcpConnectorDTO.class), anyString(), anyString());
    ...
    verify(googleFunctionCommandTaskHelper).getFunction(eq(""), isA(GcpConnectorDTO.class), anyString(), anyString());
    verify(googleFunctionCommandTaskHelper, times(2)).parseStringContentAsClassBuilder(anyString(), any(), anyString());
  ...
  public void executeTaskInternalFirstDeploymentTest() throws Exception {
    ...
    assertThat(response).isInstanceOf(GoogleFunctionRollbackResponse.class);
    assertThat(response.getCommandExecutionStatus()).isEqualTo(CommandExecutionStatus.SUCCESS);
    verify(googleFunctionCommandTaskHelper, times(1)).parseStringContentAsClassBuilder(anyString(), any(), anyString());
  }
}
