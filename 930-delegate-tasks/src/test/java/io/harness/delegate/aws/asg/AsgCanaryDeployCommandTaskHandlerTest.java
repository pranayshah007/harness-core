...
@Mock CreateAutoScalingGroupRequest createAutoScalingGroupRequest;
  ...
  public void executeTaskInternalTest() throws Exception {
    ...
    AsgCanaryDeployRequest asgCanaryDeployRequest = AsgCanaryDeployRequest.builder()
    ...
    doReturn("launchTemplateContent")
            .when(asgTaskHelper)
            .getAsgLaunchTemplateContent(asgStoreManifestsContent);

    doReturn("configurationContent")
            .when(asgTaskHelper)
            .getAsgConfigurationContent(asgStoreManifestsContent);

    doReturn("asg")
            .when(createAutoScalingGroupRequest)
    ...
    assertThat(asgCanaryDeployResponse.getAsgCanaryDeployResult().getRegion()).isEqualTo("us-east-1");

    verify(deployLogCallback)
        .saveExecutionLog(color(format("Deployment Finished Successfully"), LogColor.Green, LogWeight.Bold), LogLevel.INFO,
