...
public class CVNGNotificationChannelUtils {
  NotificationRequest.UserGroup getUserGroups(
    ...
    if (identifierRef.getScope() == Scope.ACCOUNT) {
      return NotificationRequest.UserGroup.newBuilder().setIdentifier(identifierRef.getIdentifier()).build();
    }
    if (identifierRef.getScope() == Scope.ORG) {
      return NotificationRequest.UserGroup.newBuilder()
