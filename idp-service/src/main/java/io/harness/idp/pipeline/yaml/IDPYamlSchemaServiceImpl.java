...
package io.harness.idp.pipeline.yaml;
...
public class IDPYamlSchemaServiceImpl implements IDPYamlSchemaService {
    ...
    public PartialSchemaDTO getStageYamlSchemaUtil(String accountIdentifier, String projectIdentifier,
                                                   String orgIdentifier, Scope scope, List<YamlSchemaWithDetails> stepSchemaWithDetails) {
        JsonNode idpStageSchema =
                yamlSchemaProvider.getYamlSchema(EntityType.IDP_STAGE, orgIdentifier, projectIdentifier, scope);
        JsonNode idpStageSteps =
                yamlSchemaProvider.getYamlSchema(EntityType.PLUGIN, orgIdentifier, projectIdentifier, scope);

        JsonNode definitions = idpStageSchema.get(DEFINITIONS_NODE);
        JsonNode idpStepDefinitions = idpStageSteps.get(DEFINITIONS_NODE);

        JsonNodeUtils.merge(definitions, idpStepDefinitions);
        ...
        yamlSchemaGenerator.modifyRefsNamespace(idpStageSchema, IDP_NAMESPACE);
        ...
        YamlSchemaUtils.addOneOfInExecutionWrapperConfig(idpStageSchema.get(DEFINITIONS_NODE),
        ...
        if (stepSchemaWithDetails != null) {
            YamlSchemaUtils.addOneOfInExecutionWrapperConfig(idpStageSchema.get(DEFINITIONS_NODE),
                    stepSchemaWithDetails, ModuleType.IDP, enabledFeatureFlags, Collections.emptyMap());
        }

        ObjectMapper mapper = SchemaGeneratorUtils.getObjectMapperForSchemaGeneration();
        JsonNode node = mapper.createObjectNode().set(IDP_NAMESPACE, definitions);

        JsonNode partialCiSchema = ((ObjectNode) idpStageSchema).set(DEFINITIONS_NODE, node);
