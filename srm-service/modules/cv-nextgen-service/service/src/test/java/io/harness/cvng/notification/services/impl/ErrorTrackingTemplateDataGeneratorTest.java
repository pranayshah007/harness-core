...
public class ErrorTrackingTemplateDataGeneratorTest {
  ...
  public void getTemplateDataTest() throws IllegalAccessException {
    ProjectParams projectParams = ProjectParams.builder().accountIdentifier(ACCOUNT_IDENTIFIER).projectIdentifier(PROJECT_IDENTIFIER).orgIdentifier(ORGANIZATION_IDENTIFIER).build();

    ErrorTrackingTemplateDataGenerator errorTrackingTemplateDataGenerator = spy(new ErrorTrackingTemplateDataGenerator());
    ...
    when(nextGenService.getProject(ACCOUNT_IDENTIFIER, ORGANIZATION_IDENTIFIER, PROJECT_IDENTIFIER)).thenReturn(projectDTO);
    when(nextGenService.getService(ACCOUNT_IDENTIFIER, ORGANIZATION_IDENTIFIER, PROJECT_IDENTIFIER, SERVICE_IDENTIFIER)).thenReturn(serviceResponseDTO);
    ...
    final Map<String, String> templateData = errorTrackingTemplateDataGenerator.getTemplateData(
            projectParams, MONITORED_SERVICE_NAME, MONITORED_SERVICE_ID, SERVICE_IDENTIFIER, codeErrorCondition, notificationDataMap);
    ...
    assertThat(templateData.get(EMAIL_MONITORED_SERVICE_NAME_HYPERLINK)).contains(MONITORED_SERVICE_ID).contains(MONITORED_SERVICE_NAME);
    assertThat(templateData.get(EMAIL_NOTIFICATION_NAME_HYPERLINK)).contains(NOTIFICATION_URL_VALUE).contains(NOTIFICATION_NAME_VALUE);
