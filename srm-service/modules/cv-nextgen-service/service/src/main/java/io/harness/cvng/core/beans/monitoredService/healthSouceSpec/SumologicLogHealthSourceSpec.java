...
public class SumologicLogHealthSourceSpec extends HealthSourceSpec {
  @NotNull String feature;
  @NotNull @NotEmpty @Valid List<SumologicLogHealthSourceSpec.QueryDTO> queries;
  ...
  public HealthSource.CVConfigUpdateResult getCVConfigUpdateResult(String accountId, String orgIdentifier,
    ...
    Map<Key, SumologicLogCVConfig> existingConfigMap = getExistingCVConfigMap(existingCVConfigs);
    Map<SumologicLogHealthSourceSpec.Key, SumologicLogCVConfig> currentConfigMap = getCurrentCVConfigMap(
        accountId, orgIdentifier, projectIdentifier, monitoredServiceIdentifier, identifier, name);

    Set<SumologicLogHealthSourceSpec.Key> deleted =
        Sets.difference(existingConfigMap.keySet(), currentConfigMap.keySet());
    Set<SumologicLogHealthSourceSpec.Key> added =
        Sets.difference(currentConfigMap.keySet(), existingConfigMap.keySet());
    Set<SumologicLogHealthSourceSpec.Key> updated =
  ...
  private SumologicLogHealthSourceSpec.Key getKeyFromCVConfig(@NotNull SumologicLogCVConfig cvConfig) {
    return SumologicLogHealthSourceSpec.Key.builder()
        .monitoredServiceIdentifier(cvConfig.getMonitoredServiceIdentifier())
        .queryName(cvConfig.getQueryName())
        .build();
  }

  private Map<Key, SumologicLogCVConfig> getExistingCVConfigMap(List<CVConfig> existingCVConfigs) {
    return ((List<SumologicLogCVConfig>) (List<?>) existingCVConfigs)
        .stream()
  ...
  private Map<SumologicLogHealthSourceSpec.Key, SumologicLogCVConfig> getCurrentCVConfigMap(String accountId,
