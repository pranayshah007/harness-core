...
public class UserInfoServiceImpl implements UserInfoService {
  ...
  public UserInfo getCurrentUser() {
    ...
    if (userEmail.isPresent()) {
      Optional<UserInfo> userInfo = NGRestUtils.getCgResponse(userClient.getUserByEmailId(userEmail.get()));
      return userInfo.get();
    } else {
  ...
  public UserInfo update(UserInfo userInfo, String accountIdentifier) {
    if (PrincipalType.USER.equals(SourcePrincipalContextBuilder.getSourcePrincipal().getType())) {
      ...
      Optional<UserInfo> optionalTargetUserInfo =
          NGRestUtils.getCgResponse(userClient.getUserByEmailId(userInfo.getEmail()));
    ...
    Optional<UserInfo> updatedUserInfo = NGRestUtils.getCgResponse(userClient.updateUser(userInfo));
  ...
  public TwoFactorAuthSettingsInfo getTwoFactorAuthSettingsInfo(TwoFactorAuthMechanismInfo twoFactorAuthMechanismInfo) {
    Optional<String> userEmail = getUserEmail();
    if (userEmail.isPresent()) {
      Optional<TwoFactorAuthSettingsInfo> twoFactorAuthSettingsInfo = NGRestUtils.getCgResponse(
  ...
  public UserInfo updateTwoFactorAuthInfo(TwoFactorAuthSettingsInfo authSettingsInfo) {
    ...
    if (userEmail.isPresent()) {
      Optional<UserInfo> userInfo =
          NGRestUtils.getCgResponse(userClient.updateUserTwoFactorAuthInfo(userEmail.get(), authSettingsInfo));
      return userInfo.get();
    } else {
      throw new IllegalStateException("user login required");
    }
  ...
  public UserInfo disableTFA() {
    Optional<String> userEmail = getUserEmail();
    if (userEmail.isPresent()) {
      Optional<UserInfo> userInfo = NGRestUtils.getCgResponse(userClient.disableUserTwoFactorAuth(userEmail.get()));
      return userInfo.get();
    } else {
      throw new IllegalStateException("user login required");
    }
  ...
  public PasswordChangeResponse changeUserPassword(PasswordChangeDTO passwordChangeDTO) {
    ...
    return NGRestUtils.getCgResponse(userClient.changeUserPassword(user.getUuid(), passwordChangeDTO));
  }
  ...
  public UserInfo unlockUser(String userId, String accountId) {
    ...
    if (userMetadataOpt.isPresent()) {
      ...
      Optional<UserInfo> userInfo = NGRestUtils.getCgResponse(userClient.unlockUser(email, accountId));
      return userInfo.get();
    } else {
