...
public class NgUserServiceImplTest extends CategoryTest {
  ...
  private void preAddUserToScope(String userId, Scope scope, List<RoleBinding> roleBindings, List<String> userGroups) {
    ...
    doNothing()
        .when(ngUserService)
        .addUserToScopeInternal(userId, UserMembershipUpdateSource.USER, scope, getDefaultManagedRoleIdentifier(scope));

    parentScopes.forEach(parentScope
        -> doNothing()
               .when(ngUserService)
               .addUserToScopeInternal(
                   userId, UserMembershipUpdateSource.USER, parentScope, getDefaultManagedRoleIdentifier(parentScope)));
    doNothing()
        .when(ngUserService)
